...
public class XSSFChartNumDataSource<T> implements ChartDataSource<T> {
	...
	public int getPointCount() {
		if (val != null) {
			if (isReference()) {
				return (int) val.getNumRef().getNumCache().getPtCount()
						.getVal();
			} else {
	...
	public T getPointAt(int index) {
		if (val != null) {
			if (isReference()) {
				return (T) Double.valueOf(val.getNumRef().getNumCache()
						.getPtArray(index).getV());
			} else {
				return (T) Double.valueOf(val.getNumLit().getPtArray(index)
						.getV());
			}
		}
		return null;
