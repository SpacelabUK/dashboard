...
public class StorePlans extends SplabHttpServlet {
            ...
            for (String key : blockz.keySet()) {
                ...
                if (spaceMap.containsKey(alias)) {
                    ...
                    try {
                        ...
                        File planDir = new File(PLANS_DIR);
                        if (!planDir.exists())
                            planDir.mkdirs();

                        File csvPlan = new File(PLANS_DIR + newFileName);
                        if (csvPlan.exists())
                            csvPlan.delete();

                        csvPlan.createNewFile();

                        IOUtils.writeLines(block.getAsCSV(), "\n",
                                new FileOutputStream(csvPlan));
     ...
     *
     * @param base  The base trunk where the odd depth children will go
     * @param tree  The currently examined tree
