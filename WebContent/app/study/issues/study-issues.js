(function () {
    "use strict";
    angular.module('app.study.issues').controller('StudyIssuesController',
        StudyIssuesController);
    StudyIssuesController.$inject = [
        '$stateParams', '$modal', '$timeout', 'StudyFactory', 'dataService',
        '$http',
    ];
    function StudyIssuesController($stateParams, $modal, $timeout, StudyFactory,
                                   dataService, $http) {
        var vm = this;
        vm.id = $stateParams.studyid;
        if (!vm.id)
            return;
        // vm.viewAlias = $stateParams.viewAlias;
        vm.issue = $stateParams.issue;
        vm.activityValues = [];
        vm.activityLabels = [
            'walking', 'standing', 'sitting'
        ];
        vm.isNumber = function (metric) {
            return !metric.solution || !metric.solution.type ||
                metric.solution.type == 'number';
        };
        vm.isTable = function (metric) {
            return metric.solution && metric.solution.type &&
                metric.solution.type == 'table';
        };
        vm.isList = function (metric) {
            return metric.solution && metric.solution.type &&
        ...
        vm.toAliasString = function (name) {
        ...
        vm.isHovered = function (metric) {
            return vm.hovered && vm.hovered === metric;
        };
        vm.toggle_no_of_decimals = function (metric) {
            if (metric.no_of_decimals && typeof metric.no_of_decimals === 'string' &&
        ...
        vm.downloadEMF = function (id) {
            ...
            dataService.convertSVGtoEMF().then(
        ...
        vm.copyToClipboard = function (metric, id) {
            vm.toggle_no_of_decimals(metric);
            $timeout(
                function () {
                    var nid = vm.toAliasString(id);
        ...
        vm.getSorterOrNameOrAlias = function (e) {
        ...
        vm.getCellContent = function (e, key, property) {
        ...
        vm.getContent = function (e) {
            if (e && e.solution && e.solution.content !== null &&
                ...
                if (e.solution.content !== 'no data' && e.no_of_decimals !== null &&
                    e.no_of_decimals != undefined && e.no_of_decimals != 'all' &&
                    e.solution.content.toFixed) {
        ...
        dataService
            ...
            function (response) {
                ...
                for (i = 0; i < vm.wantedMetrics.length; i++) {
                    for (j = 0; j < vm.wantedMetrics[i].metrics.length; j++)
                        allMetrics.push(vm.wantedMetrics[i].metrics[j].alias);
                }
                vm.study = StudyFactory.fetchStudy(vm.id, allMetrics);
            });
        vm.getSQM = function (number) {
            return number.toFixed(1) + 'm' + '\xB2';
        };
    }
})();
angular
    .module('app.study.issues')
    .factory(
    'StudyFactory',
    [
        'dataService',
        ...
        function (dataService, $q, $http) {
            ...
            var fetchMetric = function (measureFunc) {
                ...
                if (measureFunc.proc) {
                    ...
                    getter = dataService.resolveProc(measureFunc.proc,
                        paramString);
                } else
            ...
            var out = {
                    ...
                    $q.all(promises).then(function (response) {
                        ...
                        var storeMetrics = function () {
                            var store = function (metric) {
                                return dataService.storeMetric(metric);
                            };
                        ...
                        var storeIssues = function () {
                            var store = function (issue) {
                                return dataService.storeIssue(issue)
                            };
                            var breakPoint = 1000;
                            var lel = function (lel, i) {
                                var m = {};
                        ...
                        var storeFunctions = function () {
                            var store = function (func) {
                                return dataService.storeFunction(func);
                            };
                ...
                fetchStudy: function (id, wantedMetrics) {
                    ...
                    var promises = [];
                    // promises.push($http.get("studies/originalMetrics.json"));
                    promises.push(dataService.getMetricsDetails({
                ...
                testMetric: function (id, metricTree) {
                    ...
                    function extractMetricInputs(inputs, depth) {
                        ...
                        for (var i = 0; i < inputs.length; i++) {
                            var input = inputs[i];
                            if (input.ilk === "i" && !args.hasOwnProperty(input.datum)) {
                                return {
                    ...
                    var promises = [];
                    // promises.push($http.get("studies/originalMetrics.json"));
                    promises.push(dataService.getMetrics({
                        wanted_metrics: wantedMetrics,
