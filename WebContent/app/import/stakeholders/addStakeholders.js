		...
		.controller(
				...
				[
						...
						function($scope, $modalInstance, $http, $modal, $q, study,
								FileUploader, MatcherFactory, HTTPFactory, modalFactory,
								textParserService) {
							"use strict";
							$scope.study = study;
							...
							function getXLSXData(workbook) {
								...
								if (!workbook.Sheets[teamSheet] ||
										!workbook.Sheets[questionSheet])
							...
							uploader.onCompleteItem = function(item, response, status,
								...
								modalFactory.openSelectFileModal("Select staff list file",
										selectStaffFileDescription, ".vna").result
										.then(
												function(staffListUploader) {
													var reader = new FileReader();
													reader.onload = function(e) {
														var data = e.target.result;
														var vnadata = textParserService.getVNAData(data);
														var obj = textParserService
																.extractVNARowsUnderHeaders(vnadata.node_data,
																		[
																				"name", "id"
																		]);
														var j;
														...
														MatcherFactory.openMatcherModal("stakeholder",
																"stakeholders", response[newData], obj, {
																	prematch : 'ignorecase',
																	fromPrematch : 'alias',
																	toPrematch : 'alias',
																...
																.then(
																		function(stakeholder_message) {
																			console.log(stakeholder_message);
																			var newData = "DEPARTMENT_LIST";
																			var dbData = "DATABASE_TEAMS";
																			MatcherFactory.openMatcherModal("team",
																					"teams", response[newData],
																					response[dbData]).result
																					.then(
