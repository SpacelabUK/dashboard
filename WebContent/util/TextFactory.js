		...
		[
			function() {
				var pub = {
					getVNAData : function(text) {
						...
						for (i = 0; i < lines.length; i++) {
							if (lines[i].trim().toUpperCase() === '*NODE DATA') {
								...
								if (startsWith(lines[i + 1], 'FROM TO "') ||
										vnaFormat === 'ucinet') {
					...
					/**
					 * @returns object with keys for each header. The data under the
					 *          header is in result[key].data (array)
					 * 
					 */
