...
app.controller('addPolygonsInstance', [
		'$scope',
		'$modalInstance',
		'$http',
		'$modal',
		...
		function($scope, $modalInstance, $http, $modal, $q, study, FileUploader,
			...
			function getData(text) {
				...
				// });
				/*
				 * var blob = new Blob([ JSON.stringify(o) ], { type : "plain/text" });
				 * blob.name = 'file.json'; var flow = new Flow({ method : 'octet',
				 * target : HTTPFactory.getBackend() + 'FlowUpload' });
				 * flow.addFile(blob); flow.upload(); flow.on('fileSuccess',
				 * function(file, message) { console.log(file, message); });
				 */
					...
					var directiveDefinitionObject = {
						...
						link : function(scope, element, attrs) {
							...
							var svg = d3.select(element[0]).append("svg:svg").attr('height',
									'500px').attr('width', '100%').append("g").attr("transform",
									"translate(" + margin.left + "," + margin.right + ")").call(
									zoom);
