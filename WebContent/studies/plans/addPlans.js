...
app
		.factory(
				...
				function($q, $http) {
					...
					eql = function(str1, str2) {
						return str1.trim().toLowerCase() == str2.toLowerCase();
					}
					restructureDXF = function(dxf, selectedSpaces, part) {
							...
							if (0 == param) {
								...
								if (eql(data, "ENDTAB") || eql(data, "ENDBLK")
									...
									if (eql(currObject.type, part)
											...
											// && (
											eql(currObject.type, "BLOCK") || eql(
													currObject.type, "POLYLINE"))))
					...
					restructureSpaceTypePoly = function(o) {
						var result = {
							layers : [],
							limits : {
								max : {
						...
						for (var i = 0; i < o.c.length; i++) {
							b = o.c[i];
							var name = b.p[2].split('(')[0].trim()
									.toUpperCase();
							if (startsWith(name, genIdentifier)
									&& (endsWith(name, '-' + funcIdentifier) || endsWith(
											name, '-' + teamIdentifier))) {
								var layer = {
								...
								for (var j = 0; j < b.c.length; j++) {
									...
									if (p.type == 'POLYLINE') {
										var layr = p.p[8];
										var type;
										if (startsWithIgnoreCase(layr,
												genIdentifier + funcIdentifier
														+ '-')) {
											type = layr
													.slice((genIdentifier.length
															+ funcIdentifier.length + 1));

										} else if (startsWithIgnoreCase(layr,
												genIdentifier + teamIdentifier
														+ '-')) {
											type = layr
													.slice((genIdentifier.length
										...
										if (type) {
											...
											for (var k = 0; k < p.c.length; k++) {
												var v = p.c[k];
												if (v.type == 'VERTEX') {
													var x = parseFloat(v.p[10]) * 0.01;
													var y = parseFloat(v.p[20]) * 0.01;
										...
										if (type) {
											var points = [];
											for (var k = 0; k < p.p[10].length; k++) {
												// var v = p.c[k];
												if (v.type == 'VERTEX') {
													var x = parseFloat(p.p[10][k]) * 0.01;
													var y = parseFloat(p.p[20][k]) * 0.01;
													if (x > result.limits.max.x)
														result.limits.max.x = x;
													if (x > result.limits.max.y)
														result.limits.max.y = y;
					...
					public = {
						...
						uploadSpaces : function(study, dxf, selectedSpaces) {
							...
							for (var i = 0; i < plan.c.length; i++) {
								...
								// -1
									...
									plan.c[i].alias = plan.c[i].p[2].substring(
											2).split('\x28')[0].trim();
						...
						refreshSpaces : function(study) {
							var url = backend + 'GetAll?t=spaces&studyid='
									+ study.id;
		...
		.controller(
				...
				[
						...
						function($scope, $modalInstance, $http, $modal, $q,
								study, FileUploader, PlanFactory, HTTPFactory,
								MatcherFactory) {
							$scope.study = study;
							...
							function afterSpaceExtraction(data) {
								...
								openPlanDataModal(data).result.then(function(
										response) {
									console.log(response);
									HTTPFactory.backendPost('StorePlans',
											response);
									$modalInstance.close();
								}, function(error) {
									console.log(error);
							...
							$scope.attach = function(study) {
								...
								HTTPFactory.backendPost("StorePlans", data)
							...
							openPlanDataModal = function(planData) {
								var promise = $modal
										.open({
		...
		function($scope, $modalInstance, MatcherFactory, planData) {
			...
			if (planData.spaces && planData.spaces.length > 0) {
				$scope.dataTypes.push({
					...
					state : planData.spacesInDB.length == 0 ? 'new'
							: 'overwrite',
					refine : function() {
